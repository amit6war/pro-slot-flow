...
SELECT 
  ...
  up.phone,
  CASE 
    WHEN up.full_name = 'John Smith' THEN 'john.smith@example.com'
    WHEN up.full_name = 'Sarah Johnson' THEN 'sarah.johnson@example.com'
    WHEN up.full_name = 'Mike Davis' THEN 'mike.davis@example.com'
    ELSE 'provider@example.com'
  END,
  up.address,
  'approved',
  4.5 + (RANDOM() * 0.5),
...
AND up.business_name IN ('Smith Home Services', 'Johnson Cleaning Co', 'Davis Electrical Services')
AND NOT EXISTS (
...
SELECT 
  ...
  GREATEST(sd.min_price + (RANDOM() * (sd.max_price - sd.min_price)), 100),
...
BEGIN
  FOR provider_record IN 
    ...
    AND business_name IN ('Smith Home Services', 'Johnson Cleaning Co', 'Davis Electrical Services')
  LOOP
